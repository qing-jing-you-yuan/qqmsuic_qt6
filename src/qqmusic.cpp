...
void QQMusic::initUi()
{
    ...
    effect->setColor(QColor(0x00, 0x00, 0x00));
    ...
    connect(ui->mLike, &btFrom::click, this, [ = ]()
    {
        ui->PageStacked->setCurrentIndex(4);
    });
...
void QQMusic::on_Volume_clicked()
{
    ...
    vtool->show();
    connect(vtool, &volumeTool::getState_sig, this, [ = ](bool state)
    {
        if (state)
        {
    ...
    connect(vtool, &volumeTool::setVolume_sig, this, [ = ](int num)
    {
        qInfo() << "设置音量" << num ;
